...
function updateBadge() {
	localStore?.get([INSPECT_STATUS], (result) => {
		...
		localStore?.get([INSPECT_TAB], (result2) => {
			...
			if (
				(inspectStatus === STATUS_START && inspectingTab) ||
				(inspectStatus === STATUS_MIDDLE_START && inspectingTab)
			) {
 ...
 * @param {*} url Its a web address
 */
function createOpenURLEntry(url, isInspectInMiddle) {
	if (url && !isInspectInMiddle) {
		...
		chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
			...
			loadListener(tabs[0]?.url ? tabs[0]?.url : url, true);
			updateBadge();
		});
	} else {
